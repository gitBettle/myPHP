...
class MFilter
{
    ...
    public static function safe($string,$arr=[]) {
        ...
        if(!empty($arr))$string = str_replace($arr,'',$string);

        return $string;
    }
    public static function safedir($str,$default='',$addOn=array()){
        preg_replace('%[/\\]{1,}%', '/', $str);
        $arr=array('./','.\\','..\\','../');
        $arr=array_merge($arr,$addOn);
        return trim(str_replace($arr,$default,$str),'/');
    }
    public static function safeCls($str){
        return self::safedir($str,'/');
    }
}